  ...
  return (
    <div className="pb-36 pt-10 px-6 animate-fade-in text-center">
       ...
       <div className="glass-card p-5 rounded-xl border border-white/10 mb-6 text-left">
          ...
          <div className="space-y-3">
             <input 
                ...
                className="w-full bg-[#05010D] border border-white/10 rounded-lg p-3 text-sm text-white placeholder-gray-600 focus:border-ios-lilac outline-none transition-colors"
             />
             <input 
                type="text" 
                name="firstName"
          ...
          <button 
             onClick={handleSave}
             className="w-full mt-4 bg-white/5 hover:bg-white/10 border border-white/10 text-white text-xs font-bold py-3 rounded-lg flex items-center justify-center gap-2 transition-all active:scale-95"
          >
       ...
       <div className="space-y-3 mb-8">
           <a 
              href="https://affiliate.p.finance/ru" 
              target="_blank" 
              rel="noopener noreferrer"
              className="w-full bg-gradient-to-r from-blue-600 to-blue-500 h-14 rounded-xl flex items-center justify-center gap-2 shadow-lg shadow-blue-900/20 active:scale-95 transition-transform"
           >
           ...
           <a 
              ...
              target="_blank" 
              rel="noopener noreferrer"
              className="w-full bg-[#120B1F] border border-ios-lilac/30 h-14 rounded-xl flex items-center justify-center gap-2 active:scale-95 transition-transform"
           >
       ...
       {/* 4. СЕРТИФИКАТ (ПОЯВЛЯЕТСЯ ПРИ 100%) */}
       {progress === 100 ? (
          <div className="animate-fade-in">
             {!showCertificate ? (
                <button 
                   onClick={() => setShowCertificate(true)}
                   className="w-full bg-gradient-to-r from-amber-300 to-amber-500 h-14 rounded-xl flex items-center justify-center gap-2 shadow-[0_0_20px_rgba(245,158,11,0.4)] animate-pulse-glow text-black font-bold uppercase tracking-widest"
                >
                ...
                <div className="relative bg-[#1a1a1a] p-1 border-4 border-amber-500 rounded-lg shadow-2xl">
                   <div className="bg-white text-black p-6 rounded-sm text-center font-serif relative overflow-hidden">
                      <div className="absolute inset-0 border-[10px] border-double border-amber-100 opacity-50 pointer-events-none"></div>
                      <Award size={40} className="mx-auto text-amber-500 mb-2" />
                      <h2 className="text-2xl font-bold uppercase mb-1 tracking-widest">Сертификат</h2>
                      <p className="text-[10px] text-gray-500 uppercase tracking-widest mb-6">Подтверждает, что</p>
                      
                      <h3 className="text-xl font-bold border-b-2 border-black inline-block px-4 pb-1 mb-6">
                         {formData.firstName || 'Имя'} {formData.lastName || 'Фамилия'}
                      </h3>
                      
                      <p className="text-xs mb-1">Успешно прошел курс</p>
                      <p className="font-bold text-sm uppercase text-amber-600 mb-6">Elite Partners Academy</p>
                      
                      <div className="flex justify-between items-end mt-4">
                         <div className="text-left">
                            <p className="text-[8px] text-gray-400 uppercase">Дата</p>
                            <p className="text-[10px] font-bold">{new Date().toLocaleDateString()}</p>
                         </div>
                         <div className="text-right">
                            <div className="w-16 h-8 bg-contain bg-no-repeat bg-center opacity-50" style={{backgroundImage: 'url(https://upload.wikimedia.org/wikipedia/commons/thumb/c/c4/Signature_sample.svg/1200px-Signature_sample.svg.png)'}}></div>
                            <p className="text-[8px] text-gray-400 uppercase border-t border-gray-300 pt-1">Подпись CEO</p>
                         </div>
                      </div>
                   </div>
                   <button 
       ...
       <div className="mt-12 opacity-30">
           <p className="text-[9px] text-ios-textSec uppercase tracking-[0.3em] font-light">
              Elite Partners ID: {formData.pocketId || '---'}
           </p>
       </div>
    </div>
  );
